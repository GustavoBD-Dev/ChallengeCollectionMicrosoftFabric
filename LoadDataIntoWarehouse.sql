...
/* 
Important: In a data warehouse, foreign key constraints are not always necessary at the table level.
While foreign key constraints can help ensure data integrity, they can also add overhead to the ETL 
(Extract, Transform, Load) process and slow down data loading. The decision to use foreign key 
constraints in a data warehouse should be based on a careful consideration of the trade-offs 
between data integrity and performance.
 */

CREATE VIEW Sales.Staging_Sales
AS
SELECT * FROM [LakeHouse].[dbo].[staging_sales];
...
WITH CategorizedSales AS (
    ...
    SELECT CASE WHEN i.ItemName LIKE '%Helmet%' THEN 'Helmet'
